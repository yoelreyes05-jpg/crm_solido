...
app.post("/ventas", async (req, res) => {
  ...
  // NCF atómico vía RPC. Se eliminó el fallback Math.random(), que emitía
  // comprobantes fuera de todo rango autorizado sin avisar.
...
app.post("/cafeteria/venta", async (req, res) => {
  ...
  // ── Comprobante de la cafetería ─────────────────────────────────────────
  // La cafetería es un contribuyente DISTINTO al taller, con su propio RNC.
  // Antes compartía la secuencia del taller: por eso los NCF B02 3, 4, 10,
  // 11, 12 y 21 del taller terminaron en cafeteria_ventas.
  //
  // Mientras la cafetería no tenga RNC ni autorización de la DGII, NO puede
  // emitir comprobantes fiscales. Vende con ticket interno y ncf = null.
  // El día que tenga su autorización, se activa su fila en ncf_config
  // (unidad_negocio='cafeteria') y empieza a facturar sin tocar este código.
  //
  // Se eliminó el valor por defecto `tipo + "00000001"`, que le asignaba el
  // MISMO NCF a todas las ventas cuando no encontraba configuración.
  let ncf = null;
  let ncfTipoFinal = null;

  const { data: compData, error: errNcf } = await supabase
    .rpc("siguiente_comprobante", { p_tipo: tipo, p_unidad: "cafeteria" });
  ...
  const filaBase = { total: Number(total), metodo_pago, ncf, ncf_tipo: ncfTipoFinal, created_at: new Date() };
  ...
  // `fiscal: false` => la impresión debe mostrar TICKET INTERNO, sin la
  // leyenda "Comprobante Fiscal" y sin RNC. No es un documento fiscal.
  res.json({
    ...venta[0],
    ncf,
    fiscal: !!ncf,
    ticket: `TICKET-${String(venta[0].id).padStart(5, "0")}`
  });
});
...
app.get("/ncf/siguiente", async (req, res) => {
  const { tipo, unidad } = req.query;
  // Desde la separación taller/cafetería hay varias filas por tipo, así que
  // el filtro por unidad_negocio es obligatorio: sin él, .single() falla.
  const { data } = await supabase
    .from("ncf_config")
    .select("*")
    .eq("tipo", tipo || "B02")
    .eq("unidad_negocio", unidad || "taller")
    .eq("activo", true)
    .maybeSingle();

  // Sin secuencia activa no se inventa un número: se informa el estado real.
  if (!data) {
    return res.json({
      ncf: null,
      disponible: false,
      motivo: `No hay secuencia activa de tipo ${tipo || "B02"} para la unidad ${unidad || "taller"}.`
    });
  }

  const siguiente = (data.secuencia_actual || 0) + 1;
  const agotado = data.secuencia_hasta != null && siguiente > data.secuencia_hasta;
  const vencido = data.fecha_vencimiento != null &&
                  new Date(data.fecha_vencimiento) < new Date();

  res.json({
    ncf: data.prefijo + String(siguiente).padStart(data.longitud_secuencia || 8, "0"),
    disponible: !agotado && !vencido,
    motivo: agotado ? "Rango agotado: solicitar nueva autorización a la DGII."
          : vencido ? `Autorización vencida el ${data.fecha_vencimiento}.`
          : null,
    restantes: data.secuencia_hasta != null ? data.secuencia_hasta - siguiente + 1 : null
  });
});
...
app.post("/facturas", async (req, res) => {
  ...
  try {
    ...
    // ── NCF ────────────────────────────────────────────────────────────────
    // Se pide lo más tarde posible, justo antes del insert, para reducir al
    // mínimo la ventana en que un número puede quemarse.
    //
    // siguiente_comprobante() es atómico: usa UPDATE ... RETURNING, que
    // bloquea la fila. Dos facturas simultáneas NUNCA reciben el mismo NCF.
    // Valida además rango agotado y autorización vencida.
    //
    // Si falla, se aborta la factura. Antes había un fallback con
    // Math.random() que inventaba comprobantes: eso generaba NCF fuera de
    // todo rango autorizado, en silencio. Es preferible no vender a emitir
    // un comprobante inválido ante la DGII.
    //
    // Si el insert falla después de obtener el número, el NCF queda sin
    // usar. NO se revierte la secuencia a propósito: decrementar es inseguro
...
app.post("/cotizaciones/:id/convertir", async (req, res) => {
  try {
    ...
    // NCF atómico vía RPC (mismo mecanismo que POST /facturas)
    const tipo_ncf = cot.ncf_tipo || "B02";
    const { data: compCot, error: errNcfCot } = await supabase
      .rpc("siguiente_comprobante", { p_tipo: tipo_ncf, p_unidad: "taller" });

    if (errNcfCot || !compCot || compCot.length === 0) {
      return res.status(500).json({
        error: `No se pudo emitir el comprobante ${tipo_ncf}: ${errNcfCot?.message || "no hay secuencia activa"}`,
        detalle: "Revisar ncf_config (unidad_negocio='taller')."
      });
    }

    const ncf = compCot[0].comprobante;
